...
export default function ContactPage() {
    ...
    const validateForm = () => {
        ...
        if (!formData.companyEmail.trim()) {
            newErrors.companyEmail = "Company email is required.";
        } else if (!/^[a-z][a-z0-9._%+-]*@[a-z0-9.-]+\.[a-z]{2,}$/.test(formData.companyEmail.trim())) {
            newErrors.companyEmail = "Please enter a valid lowercase email (e.g., user@company.com).";
        } else if (formData.companyEmail.trim().length < 6 || formData.companyEmail.trim().length > 254) {
            newErrors.companyEmail = "Email must be between 6 and 254 characters.";
        }
        if (!formData.phone.trim()) {
            newErrors.phone = "Phone number is required.";
        ...
        if (formData.website.trim() && !/^https?:\/\/[\w\-]+(\.[\w\-]+)+[/#?]?.*$/.test(formData.website.trim())) {
            newErrors.website = "Please enter a valid URL (e.g., https://example.com).";
        }
        if (formData.city.trim() && !/^[a-zA-Z\s'-]+$/.test(formData.city.trim())) {
            newErrors.city = "City must contain only letters, spaces, or hyphens.";
        }
        if (formData.province.trim() && !/^[a-zA-Z\s'-]+$/.test(formData.province.trim())) {
            newErrors.province = "State must contain only letters, spaces, or hyphens.";
        }
        if (!privacyAccepted) newErrors.privacy = "You must accept the privacy policy.";
                ...
                <div className="flex flex-col lg:flex-row  overflow-hidden">
                    ...
                    <div className="lg:w-3/5 xl:w-2/3 p-8 lg:p-12 bg-[#F6F6F6]">
                                ...
                                <div className="mb-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="relative">
                                            <label
                                                className="block mb-2 font-poppins font-normal text-[14px] md:text-[16px] lg:text-[18px] leading-[25px] tracking-[0.013em] text-black">
                                                First Name
                                            </label>
                                            <div className="relative">
                                                <input
                        ...
                        *
                                            ...
                                            {errors.companyName && (
                                                <p id="companyName-error" className="text-red-600 text-sm mt-1" role="alert">
                                                    {errors.companyName}
                                                </p>
                                            )}
                                        </div>
                                        <div>
                                            ...
                                            />
                                            {errors.website && (
                                                <p id="website-error" className="text-red-600 text-sm mt-1" role="alert">
                                                    {errors.website}
                                                </p>
                                            )}
                                        </div>
                    ...
                    <span
                      ...
                      {formData.topic}
                                        ...
                                        {isTopicOpen && (
                                            <div
                                                className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg z-10 mt-1 max-h-48 overflow-y-auto">
